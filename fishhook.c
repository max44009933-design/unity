...
struct rebindings_entry { struct rebind_msg *rebindings; size_t rebindings_nel; struct rebindings_entry *next; };
static struct rebindings_entry *_rebindings_head;

static int prepend_rebindings(struct rebindings_entry **rebindings_head, struct rebind_msg *rebindings, size_t nel) {
  struct rebindings_entry *new_entry = (struct rebindings_entry *) malloc(sizeof(struct rebindings_entry));
  if (!new_entry) return -1;
  new_entry->rebindings = (struct rebind_msg *) malloc(sizeof(struct rebind_msg) * nel);
  if (!new_entry->rebindings) { free(new_entry); return -1; }
  memcpy(new_entry->rebindings, rebindings, sizeof(struct rebind_msg) * nel);
  ...
  for (uint32_t i = 0; i < section->size / sizeof(void *); i++) {
    ...
    while (cur) {
      for (uint32_t j = 0; j < cur->rebindings_nel; j++) {
        if (strlen(symbol_name) > 1 && strcmp(&cur->rebindings[j].name[0], &symbol_name[1]) == 0) {
...
static void rebind_symbols_for_image(struct rebindings_entry *rebindings, const struct mach_header *header, intptr_t slide) {
  ...
  for (uint i = 0; i < header->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
    ...
    if (cur_seg_cmd->cmd == LC_SEGMENT_ARCH_DEPENDENT) { if (strcmp(cur_seg_cmd->segname, SEG_LINKEDIT) == 0) linkedit_segment = cur_seg_cmd; }
  ...
  for (uint i = 0; i < header->ncmds; i++, cur += cur_seg_cmd->cmdsize) {
    ...
    if (cur_seg_cmd->cmd == LC_SEGMENT_ARCH_DEPENDENT) {
      ...
      for (uint j = 0; j < cur_seg_cmd->nsects; j++) {
        ...
        if ((sect->flags & SECTION_TYPE) == S_LAZY_SYMBOL_POINTERS || (sect->flags & SECTION_TYPE) == S_NON_LAZY_SYMBOL_POINTERS) perform_rebinding_with_section(rebindings, sect, slide, symtab, strtab, indirect_symtab);
      }
    }
  }
}
...
int rebind_symbols(struct rebind_msg *rebindings, size_t rebinds_nel) {
  int retval = prepend_rebindings(&_rebindings_head, rebindings, rebinds_nel);
  if (retval < 0) return retval;
  if (!_rebindings_head->next) _dyld_register_func_for_add_image(_rebind_symbols_for_image);
  else { uint32_t c = _dyld_image_count(); for (uint32_t i = 0; i < c; i++) _rebind_symbols_for_image(_dyld_get_image_header(i), _dyld_get_image_vmaddr_slide(i)); }
  return retval;
}